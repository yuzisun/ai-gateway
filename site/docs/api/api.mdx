...
/><ApiField
  ...
  type="integer"
  required="true"
  description="RetryPriority is the retry priority of the AIServiceBackend. If the value is 0, that's the primary backend.<br />If the request fails on the primary backend, it retries on the other backends in the backendRef list in order of<br />the priority."
/>
