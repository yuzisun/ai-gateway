...
func AnthropicMessageProcessorFactory(ccm x.ChatCompletionMetrics) ProcessorFactory {
	return func(config *processorConfig, requestHeaders map[string]string, logger *slog.Logger, isUpstreamFilter bool) (Processor, error) {
		...
		return &anthropicMessageProcessorUpstreamFilter{
			config:         config,
			requestHeaders: requestHeaders,
			logger:         logger,
			metrics:        ccm,
...
// anthropicProcessorRouterFilter implements [Processor] for the `/v1/messages` endpoint.
//
// This is primarily used to select the route for the request based on the model name.
type anthropicMessageProcessorRouterFilter struct {
...
func (c *anthropicMessageProcessorRouterFilter) ProcessResponseHeaders(ctx context.Context, headerMap *corev3.HeaderMap) (*extprocv3.ProcessingResponse, error) {
...
func (c *anthropicMessageProcessorRouterFilter) ProcessResponseBody(ctx context.Context, body *extprocv3.HttpBody) (*extprocv3.ProcessingResponse, error) {
	// If the request failed to route and/or immediate response was returned before the upstream filter was set,
	// c.upstreamFilter can be nil.
	if c.upstreamFilter != nil { // See the comment on the "upstreamFilter" field.
...
func (c *anthropicMessageProcessorRouterFilter) ProcessRequestBody(_ context.Context, rawBody *extprocv3.HttpBody) (*extprocv3.ProcessingResponse, error) {
	model, body, err := parseAnthropicMessageBody(rawBody)
	if err != nil {
...
// anthropicProcessorUpstreamFilter implements [Processor] for the `/v1/messages` endpoint at the upstream filter.
//
// This is created per retry and handles the translation as well as the authentication of the request.
type anthropicMessageProcessorUpstreamFilter struct {
	logger                 *slog.Logger
	config                 *processorConfig
	requestHeaders         map[string]string
...
func (c *anthropicMessageProcessorUpstreamFilter) selectTranslator(out filterapi.VersionedAPISchema) error {
	switch out.Name {
	...
	case filterapi.APISchemaGCPAnthropic:
		c.translator = translator.NewMessageAnthropicToGCPAnthropicTranslator(out.Version, c.modelNameOverride)
	default:
...
// to the extproc.
func (c *anthropicMessageProcessorUpstreamFilter) ProcessRequestHeaders(ctx context.Context, _ *corev3.HeaderMap) (res *extprocv3.ProcessingResponse, err error) {
	defer func() {
		if err != nil {
...
// ProcessRequestBody implements [Processor.ProcessRequestBody].
func (c *anthropicMessageProcessorUpstreamFilter) ProcessRequestBody(context.Context, *extprocv3.HttpBody) (res *extprocv3.ProcessingResponse, err error) {
	panic("BUG: ProcessRequestBody should not be called in the upstream filter")
}

// ProcessResponseHeaders implements [Processor.ProcessResponseHeaders].
func (c *anthropicMessageProcessorUpstreamFilter) ProcessResponseHeaders(ctx context.Context, headers *corev3.HeaderMap) (res *extprocv3.ProcessingResponse, err error) {
	defer func() {
		if err != nil {
			c.metrics.RecordRequestCompletion(ctx, false)
		}
...
// ProcessResponseBody implements [Processor.ProcessResponseBody].
func (c *anthropicMessageProcessorUpstreamFilter) ProcessResponseBody(ctx context.Context, body *extprocv3.HttpBody) (res *extprocv3.ProcessingResponse, err error) {
	defer func() {
...
func (c *anthropicMessageProcessorUpstreamFilter) SetBackend(ctx context.Context, b *filterapi.Backend, backendHandler backendauth.Handler,
	routeProcessor Processor,
) (err error) {
	defer func() {
		c.metrics.RecordRequestCompletion(ctx, err == nil)
	}()
	rp, ok := routeProcessor.(*anthropicMessageProcessorRouterFilter)
	if !ok {
...
func (c *anthropicMessageProcessorUpstreamFilter) mergeWithTokenLatencyMetadata(metadata *structpb.Struct) {
...
func parseAnthropicMessageBody(body *extprocv3.HttpBody) (modelName string, rb *anthropic.MessageNewParams, err error) {
	...
	return string(anthropicMessageReq.Model), &anthropicMessageReq, nil
}
