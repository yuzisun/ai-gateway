...
// NewMessageAnthropicToAnthropicTranslator implements [Factory] for Anthropic to Anthropic translation.
func NewMessageAnthropicToAnthropicTranslator(apiVersion string, modelNameOverride string) AnthropicMessageTranslator {
	return &anthropicToAnthropicTranslatorMessage{
		modelNameOverride: modelNameOverride,
		path:              path.Join("/", apiVersion, "message"),
	}
}

// anthropicToAnthropicTranslatorMessage implements [Translator] for /chat/completions.
type anthropicToAnthropicTranslatorMessage struct {
	modelNameOverride string
	stream            bool
	buffered          []byte
	bufferingDone     bool
	// The path of the chat completions endpoint to be used for the request. It is prefixed with the Anthropic path prefix.
	path string
}
...
func (o *anthropicToAnthropicTranslatorMessage) ResponseError(respHeaders map[string]string, body io.Reader) (
	headerMutation *extprocv3.HeaderMutation, bodyMutation *extprocv3.BodyMutation, err error,
) {
...
func (o *anthropicToAnthropicTranslatorMessage) ResponseHeaders(map[string]string) (headerMutation *extprocv3.HeaderMutation, err error) {
	return nil, nil
}

// ResponseBody implements [Translator.ResponseBody].
func (o *anthropicToAnthropicTranslatorMessage) ResponseBody(respHeaders map[string]string, body io.Reader, _ bool) (
...
func (o *anthropicToAnthropicTranslatorMessage) extractUsageFromBufferEvent() (tokenUsage LLMTokenUsage) {
